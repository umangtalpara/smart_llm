...
export class OpenAIAdapter implements ProviderAdapter {
  ...
  async validateKey(apiKey: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`OpenAI key validation failed: ${errMsg}`);
      return false;
    }
  }
  ...
  async executeChatCompletion(apiKey: string, body: Record<string, unknown>): Promise<LlmResponse> {
    try {
      ...
      const errMsg = err instanceof Error ? err.message : String(err);
      throw new HttpException(
        `OpenAI connection failed: ${errMsg}`,
        HttpStatus.BAD_GATEWAY,
      );
    }
  }

  async executeEmbeddings(apiKey: string, body: Record<string, unknown>): Promise<LlmResponse> {
    try {
