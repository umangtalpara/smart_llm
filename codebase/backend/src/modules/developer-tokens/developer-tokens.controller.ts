...
export class DeveloperTokensController {
  ...
  @Post()
  @ApiOperation({ summary: 'Generate a new custom developer token for LLM API access' })
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Token generated successfully (revealed once)' })
  ...
  async create(
    ...
    return {
      message: 'Token generated successfully. Store it safely; it will not be displayed again.',
  ...
  @Get()
  @ApiOperation({ summary: 'List all active developer tokens for the logged-in user' })
  @ApiResponse({ status: HttpStatus.OK, description: 'List of active developer tokens' })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Token successfully revoked' })
