...
export class ApiKeysController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Key successfully registered' })
  ...
  @Get()
  @ApiOperation({ summary: 'List all registered API Keys for this user (secrets stripped)' })
  @ApiResponse({ status: HttpStatus.OK, description: 'List of registered keys' })
  async findAll(@GetUser('id') userId: string) {
    return await this.apiKeysService.findAll(userId);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Retrieve details of a single API Key (secrets stripped)' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Update limits, tags, names, or raw values of an API Key' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Key updated successfully' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Key not found' })
  async update(
    @GetUser('id') userId: string,
    @Param('id') id: string,
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Key successfully deleted' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Key not found' })
