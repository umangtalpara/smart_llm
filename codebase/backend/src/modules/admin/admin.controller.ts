...
export class AdminController {
  ...
  @ApiResponse({
    status: 200,
    description: 'System aggregates and daily metrics returned',
  })
  async getSystemStats(@Query('days') days?: string) {
    return this.adminService.getSystemStats(days ? parseInt(days, 10) : 30);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Status configuration of AI providers returned',
  })
  async getProviders() {
  ...
  @ApiResponse({
    status: 200,
    description: 'AI provider global status updated',
  })
